...
function show() {
    ...
    localStorage["stop-proxy"] && "true" == localStorage["stop-proxy"] ? (document.getElementById("startP").style.display = null,
    document.getElementById("stopP").style.display = "none") : (document.getElementById("startP").style.display = "none",
    document.getElementById("stopP").style.display = null)
}
show();
...
document.getElementById("restartP").addEventListener("click", function(e) {
    localStorage["auto-homepage"] && "true" == localStorage["auto-homepage"] ? chrome.runtime.reload() : (document.getElementById("offStatus").style.display = "block",
    document.getElementById("tips").innerHTML = "请你打开设置的主页，重启后检测<br>不到主页会激活失败！10秒后重启,请不要关闭页面",
